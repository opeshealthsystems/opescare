...
<div class="page-head">
    ...
    <p class="page-subtitle">{{ __('public.lab_portal.page_subtitle_results', [], $l) ?: 'View all resulted tests â€” filter by flag or patient.' }}</p>
</div>

<form method="GET" class="filter-bar">
    <label class="filter-search">
        <i data-lucide="search"></i>
        <input type="text" name="search" placeholder="{{ __('public.lab_portal.ph_search_results', [], $l) ?: 'Parameter or patientâ€¦' }}" value="{{ request('search') }}" aria-label="{{ __('public.aria_search_results') }}">
    </label>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr class="{{ $result->isAbnormal() ? 'row-emergency' : '' }}">
                    ...
                    <td data-label="{{ __('public.lab_portal.col_patient', [], $l) ?: 'Patient' }}">{{ $result->patient?->full_name ?? 'â€”' }}</td>
                    ...
                    <td data-label="{{ __('public.lab_portal.col_reference', [], $l) ?: 'Reference' }}" class="td-muted">{{ $result->reference_range ?? 'â€”' }}</td>
                    ...
                    <td data-label="{{ __('public.lab_portal.col_resulted_at', [], $l) ?: 'Resulted at' }}" class="td-muted">{{ $result->resulted_at?->format('d M Y H:i') ?? 'â€”' }}</td>
                </tr>
                @empty
