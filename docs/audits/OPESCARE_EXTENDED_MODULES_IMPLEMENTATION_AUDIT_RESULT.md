> ⚠️ **SUPERSEDED (2026-06-11).** This May 18 result is stale and inaccurate — it marks
> many modules as `NOT_STARTED`/`PLANNED_ONLY` that are now fully implemented (Appointments,
> Queue, Billing, Telemedicine, Ward, CDSS, Support, Data Import, Subscriptions, and more).
> Do **not** rely on it. Use [`../AS_BUILT_IMPLEMENTATION_REGISTER.md`](../AS_BUILT_IMPLEMENTATION_REGISTER.md)
> for current state and [`SPEC_VS_CODE_GAP_AUDIT.md`](SPEC_VS_CODE_GAP_AUDIT.md) for the gap list.
